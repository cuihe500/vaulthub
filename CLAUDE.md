...
37. 在编码完成后，必须执行make fmt、make line 等检查操作（单元测试默认不执行，除非显式指明）和 make swag生成文档，并且修复存在的问题。
...
52. 新增接口请求的时候请先检索request.js，不要重复出现/api等已在封装时加入的前辍
53. 调用API后必须明确数据结构，不能假设返回值就是期望的类型。例如后端返回 {data: {configs: [], total: 10}}，前端必须正确提取 result.configs 而不是直接使用 result
54. 对于列表接口，必须先查看Swagger文档确认返回格式：若返回 {list: [], total: number} 结构，则提取 list；若直接返回数组，则直接使用；禁止假设数据结构
55. 在赋值给组件响应式变量前，必须确保数据类型正确。例如表格绑定的 :data 期望数组，则必须保证变量是数组而非对象

### 通用工作流程约束

56. 任何需要现在时间的地方，都必须调用date命令获取真实的现在时间
57. 优先使用MCP能力（如搜索，第三方库检索），若MCP无法使用，再回退到原始模式
58. 在引用第三方库之前，必须完全了解该库的情况，包括实际能力、接口规范等
59. 所有文档应该正确放置在docs及其子文件夹下
60. 若该目录必须遵守其他相关的规定，则可以在该目录下新增README.md，该文档将会视为和章程一个优先级的规定
61. 若目录下存在README.md，必须先阅读并理解该文档内容，并且将其视为和章程一个优先级
62. 对于分页逻辑，若不带有分页参数，则视为全部导出
